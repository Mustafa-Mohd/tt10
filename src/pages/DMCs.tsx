...
import { Search, Award, Globe } from 'lucide-react';
...
const DMCs = () => {
  ...
  const countries = ['Spain', 'Thailand', 'France', 'Italy', 'Greece', 'Portugal'];

  const fetchListings = async () => {
    setLoading(true);
    let query = supabase.from('listings').select('*').eq('type', 'dmc');
    if (searchQuery) {
  ...
  if (!user || (userRole !== 'dmc' && userRole !== 'user')) {
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-4">
            Top <span className="text-primary">DMCs</span>
          </h1>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Connect with certified Destination Management Companies for authentic local experiences.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            ...
            <div className="col-span-full text-center py-16">
              ...
              <h3 className="text-2xl font-bold text-foreground mb-2">No DMCs found</h3>
